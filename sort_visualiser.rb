...
SCREEN_HEIGHT = 400 # Should be multiple of 100
...
class GameWindow < Gosu::Window
  ...
  def selection_sort(loop)
	  min_pos = loop
  for j in (loop + 1)..(COUNT - 1)
    if (@columns[j].value < @columns[min_pos].value)
      min_pos = j
    end
 end
  # Knowing the min, swap with current first element (at position i)
  temp = @columns[loop]
  @columns[loop] = @columns[min_pos]
  @columns[min_pos] = temp
	end
